...
class Enemy : public Actor {
  public:
    Enemy() {
        m_Type = ACTOR_ENEMY;
        ZeroMemory(&m_Data, sizeof(EnemyData));
        m_Data.characterType = 1;
        m_Data.voice = VOICE_ENEMY_0;
    ...
    void Duplicate(Actor* sourceActor) override {
        ...
        Enemy* source = (Enemy*)sourceActor;
    ...
    bool OnLoad(ChunkReader* chunk) override {
        ...
        m_Data.unk4 = chunk->Read<uint32_t>();
        m_Script.frame = GetFrame();
    ...
    void OnSave(BinaryWriter* writer) override {
        ...
        writer->WriteUInt32(m_Data.unk4);
    ...
    size_t GetDataSize() override { return sizeof(EnemyData); }

    GameScript* GetScript() override { return &m_Script; }

    EnemyData* GetData() { return &m_Data; }
