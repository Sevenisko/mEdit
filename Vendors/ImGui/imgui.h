...
struct ImGuiTextFilter;             // Helper to parse and apply text m_Filters (e.g. "aaaaa[,bbbbb][,ccccc]")
...
// Scalar data types
typedef unsigned int        ImGuiID;// A unique ID used by widgets (typically the m_Result of hashing a stack of string)
...
namespace ImGui
{
    ...
    // - For each independent axis of 'size': ==0.0f: use remaining host window size / >0.0f: fixed size / <0.0f: use remaining window size minus abs(size) / Each axis can use a different m_Mode, e.g. ImVec2(0,400).
    ...
    IMGUI_API void          PushButtonRepeat(bool repeat);                                  // in 'repeat' m_Mode, Button*() functions return repeated true in a typematic manner (using io.KeyRepeatDelay/io.KeyRepeatRate setting). Note that you can call IsItemActive() after any Button() to tell if the button is held in the current frame.
    ...
    IMGUI_API void          Separator();                                                    // separator, generally horizontal. inside a menu bar or in horizontal layout m_Mode, this becomes a vertical separator.
    ...
    IMGUI_API ImVec2        GetCursorScreenPos();                                           // cursor position in absolute coordinates (useful to work with ImDrawList API). generally top-left == GetMainViewport()->Pos == (0,0) in single viewport m_Mode, and bottom-right == GetMainViewport()->Pos+Size == io.DisplaySize in single-viewport m_Mode.
    ...
    // - In the future we will extend this concept further to also represent Platform Monitor and support a "no main platform window" operation m_Mode.
...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_CtrlEnterForNewLine = 1 << 11,  // In multi-line m_Mode, unfocus with Enter, add new line with Ctrl+Enter (default is opposite: unfocus with Ctrl+Enter, add line with Enter).
    ImGuiInputTextFlags_NoHorizontalScroll  = 1 << 12,  // Disable following the cursor horizontally
    ImGuiInputTextFlags_AlwaysOverwrite     = 1 << 13,  // Overwrite m_Mode
    ImGuiInputTextFlags_ReadOnly            = 1 << 14,  // Read-only m_Mode
    ImGuiInputTextFlags_Password            = 1 << 15,  // Password m_Mode, display all characters as '*'
...
struct ImGuiIO
{
    ...
    float       KeyRepeatDelay;                 // = 0.250f         // When holding a key/button, time before it starts repeating, in seconds (for buttons in Repeat m_Mode, etc.).
    ...
    bool        ConfigDockingNoSplit;           // = false          // Simplified docking m_Mode: disable window splitting, so docking is limited to merging multiple windows together into tab-bars.
...
// Helper: Parse and apply text m_Filters. In format "aaaaa[,bbbb][,ccccc]"
...
// In single viewport m_Mode, top-left is == GetMainViewport()->Pos (generally 0,0), bottom-right is == GetMainViewport()->Pos+Size (generally io.DisplaySize).
