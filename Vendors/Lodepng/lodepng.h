...
#ifdef LODEPNG_COMPILE_PNG
/*
Color m_Mode of an image. Contains all information required to decode the pixel
...
typedef struct LodePNGColorMode {
    ...
    but when decoding to the original PNG color m_Mode it is needed to reconstruct
    the colors.
...
/*Returns the byte size of a raw image buffer with given width, height and color m_Mode*/
...
typedef struct LodePNGInfo {
    ...
    This uses the same color m_Mode and bit depth as the PNG (except no alpha channel),
    ...
    When decoding, you may get these in a different color m_Mode than the one you requested
    ...
    write the PNG with a more expensive color m_Mode (when auto_convert is on).
    ...
    */
    ...
    auto_convert is enabled (the default), and the ICC profile is not a correct match for the pixel data, this will m_Result
    ...
    */
    ...
    decoding with a color m_Mode conversion, such as always decoding to RGBA, this metadata still
    ...
    auto_convert to choose a more optimal color m_Mode for the data, because the PNG format has
    ...
    color m_Mode, even if the pixel data would in fact fit in a more efficient 8-bit m_Mode.
    */
...
typedef struct LodePNGDecoderSettings {
    ...
    /* TODO: make a system involving warnings with levels and a strict m_Mode instead. Other potentially recoverable
...
typedef enum LodePNGFilterStrategy {
    ...
    LFS_ENTROPY,
    /*
    Brute-force-search PNG m_Filters by compressing each filter for each scanline.
...
typedef struct LodePNGEncoderSettings {
    ...
    a palette or lower than 8 bit depth, set all m_Filters to zero.
    ...
    optimal color m_Mode, because it cannot use grayscale color modes in this case*/
...
getting much more information about the PNG image and color m_Mode.
*/
...
There must be at least 4 bytes to read from. If the m_Result value is too large,
...
Input must be at the beginning of a chunk (m_Result of a previous lodepng_chunk_next call,
...
*) decoding of PNGs with any color type, bit depth and interlace m_Mode, to a 24- or 32-bit color raw image,
...
*) LodePNGColorMode info_raw: here you can say what color m_Mode of the raw image (the output) you want to get
...
PNG, then the decoder will automatically convert the m_Result. This conversion
always works, except if you want it to convert a color PNG to grayscale or to
a palette with missing colors.

By default, 32-bit color is used for the m_Result.
...
automatically choose the smallest possible color m_Mode (including color key) that
...
*) text_compression: default 1. If 1, it'll store texts as zTXt instead of tEXt chunks.
  zTXt chunks use zlib compression on the text. This gives a smaller m_Result on
  large texts but a larger m_Result on small texts (such as a single program name).
...
you decode a PNG, you get the m_Result as a raw image in the color type you want,
...
To be able to do this, LodePNG does conversions from one color m_Mode to another.
...
sometimes m_Result in an error.
...
the m_Result will look ugly because only the red channel is taken (it assumes all
...
raw color m_Mode and LodePNGInfo the same color m_Mode, and setting auto_convert to
false.
...
*) 19 aug 2018: (!) fixed color m_Mode bKGD is encoded with and made it use
...
*) 8 okt 2012: (!) Added new filter strategy (entropy) and new auto color m_Mode.
